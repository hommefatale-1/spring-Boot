...
<body>
	<div id="app">
		<li>
			<ul v-bind:class="[kind==1 ? 'select-tob' : 'tab']"
				@click="fnList(1)">공지사항
			</ul>
			<ul v-bind:class="[kind==2 ? 'select-tob' : 'tab']"
				@click="fnList(2)">자유게시판
			</ul>
			<ul v-bind:class="[kind==3 ? 'select-tob' : 'tab']"
				@click="fnList(3)">게시판
			</ul>
		</li>
		<div>
		...
		<table border="1">
			...
			<tr v-for="(item, index) in list">
				...
				<td>{{item.hit}}</td>
				<td><a href="javascript:;" @click="fnUser(item.userId)">{{item.userName}}</a>
				</td>
				<td>{{item.cdateTime}}</td>
			</tr>
		</table>
		<div>
			<button @click="fnboardAdd()">작성하기</button>
			<button @click="fnDelete()">삭제하기</button>
		</div>
	</div>
</body>
</html>
